...
describe("GenericDriver", () => {
  ...
  test("device vendor", () => expect(GenericDriver.deviceVendor).toBe(TransceiverVendor.Kenwood))
  ...
  describe("setVFO", () => {
    ...
    test("implements the command correctly", async () => {
      ...
      expect(testCommunicationDriver.write).toHaveBeenCalledWith(textEncoder.encode("FA00014250000;"))

      await genericTransceiver.sendCommand('setVFO', { frequency: 7_250_000, vfo: TransceiverVFOType.B })
      expect(testCommunicationDriver.write).toHaveBeenCalledWith(textEncoder.encode("FB00007250000;"))
    })
  ...
  describe("getVFO", () => {
    ...
    test("implements the command correctly", async () => {
      testCommunicationDriver.write.mockImplementationOnce(() => testCommunicationDriver.subject.next(textEncoder.encode(`FB00012345;FA00014250000;`)))
      await expect(genericTransceiver.sendCommand('getVFO', { vfo: TransceiverVFOType.A })).resolves.toBe(14_250_000)

      testCommunicationDriver.write.mockImplementationOnce(() => testCommunicationDriver.subject.next(textEncoder.encode(`FA00012345;FB00007200000;`)))
  ...
  describe("setAGC", () => {
    ...
    test("implements the command factory correctly", async () => {
      ...
      expect(testCommunicationDriver.write).toHaveBeenCalledWith(textEncoder.encode("GC00;"))

      await genericTransceiver.sendCommand('setAGC', { attack: TransceiverAGCAttack.Fast })
      expect(testCommunicationDriver.write).toHaveBeenCalledWith(textEncoder.encode("GC03;"))

      await genericTransceiver.sendCommand('setAGC', { attack: TransceiverAGCAttack.Mid })
      expect(testCommunicationDriver.write).toHaveBeenCalledWith(textEncoder.encode("GC02;"))

      await genericTransceiver.sendCommand('setAGC', { attack: TransceiverAGCAttack.Slow })
      expect(testCommunicationDriver.write).toHaveBeenCalledWith(textEncoder.encode("GC01;"))
    })
