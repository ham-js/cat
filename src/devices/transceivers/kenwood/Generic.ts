import { z } from "zod";
import { TransceiverAGCAttack } from "../base/TransceiverAGCAttack";
import { TransceiverDevice } from "../base/TransceiverDevice";
import { TransceiverDeviceVendor } from "../base/TransceiverDeviceVendor";
import { filter, firstValueFrom, map } from "rxjs";
import { delimiterParser } from "../../../parsers/delimiterParser";
import { TransceiverVFOType } from "../base/TransceiverVFOType";
import { TransceiverCommands } from "../base/TransceiverCommands";

const vfoType = z.enum([
  TransceiverVFOType.A,
  TransceiverVFOType.B
])

const AGCAttackNumbers: Record<TransceiverAGCAttack.Off | TransceiverAGCAttack.Slow | TransceiverAGCAttack.Mid | TransceiverAGCAttack.Fast, number> = {
  [TransceiverAGCAttack.Off]: 0,
  [TransceiverAGCAttack.Slow]: 1,
  [TransceiverAGCAttack.Mid]: 2,
  [TransceiverAGCAttack.Fast]: 3,
}

export class Generic extends TransceiverDevice {
  static readonly deviceName: string = "Generic Transceiver"
  static readonly deviceVendor = TransceiverDeviceVendor.Kenwood

  readonly _commands = {
    setAGC: Object.assign(
      ({ attack }: Parameters<NonNullable<TransceiverCommands["setAGC"]>>[0]) =>
        this.serialPort.writeString(
          `GC0${AGCAttackNumbers[attack as keyof typeof AGCAttackNumbers]};`
        ),
      {
        parameterType: z.object({
          attack: z.enum([
            TransceiverAGCAttack.Off,
            TransceiverAGCAttack.Slow,
            TransceiverAGCAttack.Mid,
            TransceiverAGCAttack.Fast,
          ])
        })
      }
    ),
    setVFO: Object.assign(
      ({ frequency, vfo }: Parameters<TransceiverCommands["setVFO"]>[0]) =>
        this.serialPort.writeString(
          `F${vfo === TransceiverVFOType.A ? 'A' : 'B'}${frequency.toString(10).padStart(11, '0')};`
        ),
      {
        parameterType: z.object({
          frequency: z
            .number()
            .int()
            .gte(30_000)
            .lte(74_800_000),
          vfo: vfoType
        })
      }),
    getVFO: Object.assign(
      async ({ vfo }: Parameters<TransceiverCommands["getVFO"]>[0]) => {
        const value = firstValueFrom(
          delimiterParser(this.serialPort.stringObservable(), ";")
            .pipe(
              map((command) => parseInt(command.match(new RegExp(`F${vfo === TransceiverVFOType.A ? 'A' : 'B'}(\\d+);`))?.[1] ?? "", 10)),
              filter(Boolean)
            )
        )

        await this.serialPort.writeString(`F${vfo === TransceiverVFOType.A ? 'A' : 'B'};`)

        return value
      },
      {
        parameterType: z.object({
          vfo: vfoType
        })
      }
    )
  }
}
