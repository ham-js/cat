...
export const supportedDrivers = <This extends new (...parameters: any[]) => any>(supportedDrivers: DriverType[]) => (target: This, context: ClassDecoratorContext<This>) => {
  return class DecoratedClass extends target {
    ...
    constructor(...parameters: any[]) {
      ...
      if (this.constructor !== DecoratedClass && "supportedDrivers" in this.constructor && !equal(supportedDrivers, this.constructor.supportedDrivers)) return
      if (!supportedDrivers.includes(this.driver.type)) throw new Error(`This device doesn't support the driver ${this.driver.type} (supported drivers: [${supportedDrivers.join(", ")}])`)
    }
